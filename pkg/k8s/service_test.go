...
func (s *K8sSuite) TestServiceString(c *check.C) {
	tests := []struct {
		...
		{
			"k8s-ipv4-only-clusterip-service",
			&slim_corev1.Service{
				ObjectMeta: slim_metav1.ObjectMeta{
					Name:      "foo",
					Namespace: "bar",
			...
			"frontends:[127.0.0.1]/ports=[]/selector=map[foo:bar]",
			true,
		},

		{
			"k8s-dual-stack-clusterip-service",
			&slim_corev1.Service{
				ObjectMeta: slim_metav1.ObjectMeta{
					Name:      "foo",
					Namespace: "bar",
			...
			"frontends:[127.0.0.1 fd00::1]/ports=[]/selector=map[foo:bar]",
			true,
		},
	}
